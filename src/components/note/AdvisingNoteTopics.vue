<template>
  <div>
    <label
      ...
      class="d-block font-size-16 font-weight-bold"
      :class="labelClass"
      :for="!readOnly && size(options) ? 'add-topic-select-list' : undefined"
    >
      Topic {{ size(options) === 1 ? 'Category' : 'Categories' }}
    </label>
    <select
      v-if="!readOnly && size(options)"
      id="add-topic-select-list"
      :key="noteStore.model.topics.length"
      v-model="selected"
      autocomplete="off"
      class="bg-white select-menu mb-1 mt-2"
      :class="{'w-100': xs}"
      :disabled="disabled"
    >
